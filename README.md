...
In this case the `getDefaultBids` operation can be called with either `VIEW_OWN_DEFAULT_BIDS` or `VIEW_ANY_DEFAULT_BIDS`. However if the Uaer has the `VIEW_OWN_DEFAULT_BIDS` _Permission_, they  **MUST** send their own [_Participant ID_](#-what-is-participant-id) in the request. Failing to do so will also result in a `403 FORBIDDEN` error response from the API.
